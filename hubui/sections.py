...
def overview(library: Library, roster: list[dict], discovery: dict,
    ...
    with ui.row().classes("w-full gap-4 no-wrap"):
        ...
        with _card("Players"):
            ui.label(str(len(roster))).classes("hub-kpi")
            ui.label("in the roster").classes("text-xs opacity-60")
...
def extensions(roster: list[dict]) -> None:
    ui.label("An extension runs either on the hub or on one player. Where it runs is a "
    ...
    with ui.element("div").classes("hub-card w-full"):
        ...
        ui.label("Install one from a repository, or drop a package here. The list will "
                 "show what it declares and which players it reached.") \
            .classes("hub-help")
