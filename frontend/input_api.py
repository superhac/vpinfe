...
def get_bindings(config) -> dict[str, list[str]]:
    ...
    for action in input_actions.actions():
        current = _raw(config, input_actions.SECTION, action.name)
        ...
        for old in action.legacy:
            ...
            if value is not None:
                found += input_actions.binding_for_legacy(old, value)
...
def get_paging_config(config):
    ...
    paging_type = cfg_get(config, input_actions.SECTION, "paging_type",
                          PAGING_TYPE_DEFAULT).strip().lower()
    if paging_type not in PAGING_TYPES:
        paging_type = PAGING_TYPE_DEFAULT
    try:
        page_size = int(cfg_get(config, input_actions.SECTION, "paging_size",
...
def get_joymapping(config):
    ...
    return {action.legacy_joy_key:
            ",".join(input_actions.pad_buttons_in(bindings[action.name]))
            for action in input_actions.actions() if action.legacy_joy_key}
...
def get_keymapping(config):
    ...
    return {action.legacy_key_key:
            ",".join(input_actions.keys_in(bindings[action.name]))
            for action in input_actions.actions() if action.legacy_key_key}
...
def set_button_mapping(iniconfig, action_name, button_index):
    ...
    """
    name = input_actions.action_for_legacy_key(action_name)
    if not name:
        return {"success": False, "message": f"Invalid action: {action_name}"}
    try:
        parser = _parser(iniconfig)
        if not parser.has_section(input_actions.SECTION):
            parser.add_section(input_actions.SECTION)
        current = get_bindings(iniconfig)[name]
        # Replace this action's gamepad binding; leave its keys and anything richer.
        kept = [b for b in current if not b.startswith(input_actions.PAD_PREFIX)]
        kept.append(f"{input_actions.PAD_PREFIX}0/button:{button_index}")
        parser.set(input_actions.SECTION, name, ",".join(kept))
        iniconfig.save()
