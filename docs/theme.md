...
| `vpin.endpoints` | `object` | Where the things this page talks to are — `{ hub, device, assets, frontend_channel }`. The first three are addresses you add a path to: `hub` is the library and what's known about it, `device` is this machine (launching, play state, hardware), `assets` is the files themselves (theme packages, table media, shared art). `frontend_channel` is not an address but a line held open — how this page and VPinFE talk to each other, both ways — so nothing is appended to it. Build URLs from these rather than assuming a host or a port: the halves can be separate machines, and only this knows where they are. |
